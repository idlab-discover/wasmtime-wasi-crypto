...
When a known gap in this file is resolved, update the relevant table entry and remove the corresponding row from the known incomplete areas table in `AGENTS.md`.
...
- `keypair_from_pk_and_sk` validates algorithm compatibility but always returns `NotImplemented` (`keypair.rs:85-100`). When fixed, add a regression test and update this entry.
...
`KxOptionsInner` has a `context` field that is never read — dead code. All `options_set` / `options_set_u64` calls return `UnsupportedOption`.

### Known gaps

- `KxOptionsInner.context` is dead code — should be removed or wired up.
- No test component exists for key exchange yet (`wasi-crypto-kx` is not written).
...
- No test component exists yet (`wasi-crypto-asymmetric-common` is not written).
...
`array_output_pull` (WIT) allocates a `Vec::new()` then passes it to `ArrayOutput::pull`. `pull` checks `buf.len() < data.len()` and returns `Overflow` if true — so for any non-empty output this always returns `Overflow`. This is a suspected bug. Investigation and a regression test in `wasi-crypto-common` are needed. When fixed, update this entry.
...
All `HostXxx::drop` impls are `todo!()` except `HostOptions::drop`. See [`architecture.md`](architecture.md) for detail.
...
| `wasi-ephemeral-crypto-signatures` | `wasi-crypto-signatures` | Partial | Ed25519, ECDSA P256/K256, RSA PKCS1 covered; managed keypairs `#[should_panic]` |
| `wasi-ephemeral-crypto-kx` | — | Missing | not yet written |
| `wasi-ephemeral-crypto-asymmetric-common` | — | Missing | not yet written |
...
**Test origin:** AI-generated from the doc comments in the WITX 0.10 spec. Not exhaustive. Future goal: port the test suite from [wasi-crypto-host-functions](https://github.com/wasm-crypto/wasi-crypto-host-functions).

**Regression tests:** when a bug is fixed, add a test in the appropriate `test-components/<interface>/src/lib.rs` before or alongside the fix.
