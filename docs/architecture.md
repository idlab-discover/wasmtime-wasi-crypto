...
The `spec/` submodule is the local copy of the WIT spec. Changes are made here first and pushed upstream later. The WIT spec itself is a direct, conservative translation of WITX 0.10 — see [PR #94](https://github.com/WebAssembly/wasi-crypto/pull/94) and the [WITX 0.10 spec](https://github.com/WebAssembly/wasi-crypto/tree/main/witx/witx-0.10) for the authoritative behavioral reference.
...
`bindings/mod.rs` contains the `bindgen!` invocation. Its `with` block maps every WIT resource to its backing Rust type. `SecretsManager` is currently **not mapped** (commented out) — it has no backing Rust type, and all `secrets_manager_*` functions return `CryptoErrno::UnsupportedFeature`. When this is resolved, update `bindings/mod.rs` and the known incomplete areas in `AGENTS.md`.
...
| Batch interfaces | Defined and usable | Defined but commented out of `world.wit` |
